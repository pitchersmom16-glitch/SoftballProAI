...
// === FEEDBACK EVENTS TABLE (Continuous Learning Foundation) ===
// Logs every Brain decision and user reaction for learning and adaptation
export const feedbackEvents = pgTable("feedback_events", {
  id: serial("id").primaryKey(),
  
  // Who provided feedback
  userId: text("user_id").notNull().references(() => users.id),
  role: text("role").notNull(), // "player", "parent", "coach", "instructor", "team_coach"
  
  // Brain decision tracking
  brainDecisionId: text("brain_decision_id").notNull(), // UUID or unique ID generated by Brain
  decisionType: text("decision_type").notNull(), // "drill", "goal", "plan", "mental_content"
  decisionRefId: integer("decision_ref_id"), // Optional: drill_id, goal_id, etc.
  
  // Context (what situation was this recommendation for)
  skillType: text("skill_type"), // "PITCHING", "HITTING", "CATCHING", "FIELDING"
  athleteAge: integer("athlete_age"), // Age at time of feedback
  athleteLevel: text("athlete_level"), // "Beginner", "Intermediate", "Advanced"
  detectedIssues: text("detected_issues").array(), // Issues the Brain was addressing
  
  // User reaction
  rating: integer("rating"), // 1-5 stars, nullable
  action: text("action").notNull(), // "accepted", "rejected", "edited", "skipped", "completed"
  pushbackText: text("pushback_text"), // Optional free-text feedback
  editedVersion: jsonb("edited_version"), // If user edited, store what they changed to
  
  // Outcome tracking (for future ML)
  outcomeStatus: text("outcome_status"), // "pending", "improved", "no_change", "worsened"
  outcomeNotes: text("outcome_notes"), // Optional notes about outcome
  outcomeDate: timestamp("outcome_date"), // When outcome was measured
...
export const insertMentalEdgeSchema = createInsertSchema(mentalEdge).omit({
  id: true,
  createdAt: true,
});
...
export const insertFeedbackSchema = createInsertSchema(assessmentFeedback).omit({
  id: true,
  createdAt: true,
});

export const insertPlayerCheckinSchema = createInsertSchema(playerCheckins).omit({
  id: true,
  createdAt: true,
});
...
export const insertCoachStudentSchema = createInsertSchema(coachStudents).omit({
  id: true,
  createdAt: true,
});

export const insertPlayerCoachRelationshipSchema = createInsertSchema(playerCoachRelationships).omit({
  id: true,
  createdAt: true,
});

export const insertCoachInviteSchema = createInsertSchema(coachInvites).omit({
  id: true,
  createdAt: true,
});

export const insertPlayerSettingsSchema = createInsertSchema(playerSettings).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertStudentInviteSchema = createInsertSchema(studentInvites).omit({
  id: true,
  createdAt: true,
});

export const insertBaselineVideoSchema = createInsertSchema(baselineVideos).omit({
  id: true,
  createdAt: true,
});

export const insertPlayerOnboardingSchema = createInsertSchema(playerOnboarding).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type CreateFeedbackEventRequest = z.infer<typeof insertFeedbackEventSchema>;
...
export const insertGameChangerStatsSchema = createInsertSchema(gameChangerStats).omit({
  id: true,
  createdAt: true,
  importedAt: true,
});

export const insertSkeletalAnalysisSchema = createInsertSchema(skeletalAnalysis).omit({
  id: true,
  createdAt: true,
});
