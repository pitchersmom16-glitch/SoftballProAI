...
export const coaches = pgTable('coaches', {
  ...
  userId: text('user_id')
    .notNull()
    .references(() => users.id), // Link to auth user
...
export const playerCoachRelationships = pgTable('player_coach_relationships', {
  ...
  playerId: text('player_id')
    .notNull()
    .references(() => users.id), // The player (user)
  coachId: integer('coach_id')
    .notNull()
    .references(() => coaches.id), // The assigned coach
...
export const coachInvites = pgTable('coach_invites', {
  ...
  fromPlayerId: text('from_player_id')
    .notNull()
    .references(() => users.id), // Player sending invite
...
export const insertPlayerCoachRelationshipSchema = createInsertSchema(
...
export type CreatePlayerCoachRelationshipRequest = z.infer<
  typeof insertPlayerCoachRelationshipSchema
>;
...
export type UpdateHomeworkRequest = Partial<CreateHomeworkRequest> & {
  status?: string;
  completedAt?: Date;
};
