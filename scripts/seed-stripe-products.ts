...
async function createProducts() {
  ...
  const eliteProduct = await stripe.products.create({
    ...
    metadata: {
      tier: 'elite',
      features:
        'All Basic features, AI Training Roadmap, GameChanger integration, Recruiting profile',
    },
  });
  ...
  const coachProduct = await stripe.products.create({
    ...
    metadata: {
      tier: 'coach',
      features:
        'All Elite features, Team roster management, Practice architect, Student lesson roster',
    },
  });
