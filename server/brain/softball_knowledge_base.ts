...
export const PITCHING_KNOWLEDGE = {
  fundamentals: {
    windmillMechanics: {
      description:
        'The windmill pitch is broken into three main segments: upper half, lower half, and arm circle',
      keyPhases: [
      ...
      criticalMechanics: {
        elbowBend:
          'Elbow must bend coming down the back side of the circle to enable proper arm whip. The elbow leads, NOT the hand.',
        internalRotation:
          'Shoulder internal rotation generates 40-60% of pitch velocity. This is THE primary velocity source.',
        dragFoot:
          'Active stabilizer maintaining ground contact. Posterior chain pulls through dirt rather than lifting.',
        hipShoulder:
          'Hips fire first, creating 40-50 degrees of separation. This loads the core like a rubber band in stretch-shortening cycle.',
      },
    },

    gripFundamentals: {
      fourSeam:
        "Primary pitch to master first. Requires 60% strike accuracy before advancing. Use 'C' seams with fingertips on seams, thumb aligned with middle finger, fingers slightly curled for spin.",
      riseBall:
        'Pointer finger dug into seam. Requires 1400+ RPM backspin with 12-6 axis. Magnus effect creates upward force.',
  ...
  expertInsights: {
    amandaScarborough: {
      philosophy:
        "There is no single 'correct' way to pitch. Elite pitchers like Monica Abbott, Cat Osterman, and Lisa Fernandez use varying techniques but all achieved Olympic-level success.",
      gloveHandVariation:
        "Abbott uses 'thumb up' position while Fernandez uses 'thumb down' - both work at elite level",
      coachingPrinciple:
        'Pitchers should adopt mechanics that feel comfortable and confident for their individual bodies rather than forcing standardized technique',
    },
    ...
    catOsterman: {
      ...
      approach:
        'Keeps mechanics simpler, throws slower (62 mph), specializes in pitches down in zone',
      strength: 'Focuses extensively on drop ball during bullpen sessions',
    },

    paisleysPitching: {
      velocityDevelopment:
        'Two primary methods: isometrics and long toss with progressive overload',
      isometrics:
        'Focus on positions like takeoff and finish - both athletic and pitching-specific',
      longToss: 'Progressive overload increasing reps and distance weekly',
      riseballTraining:
        'Requires efficient spin (over 60% backspin) with force directed under ball. Use weighted 9oz softballs and football spinning drills from whip position.',
    },
  },
    ...
    advanced: [
      {
        ...
        description:
          'Stand 45 degrees to wall, touch wall with elbow (not hand) during arm circle. Perform 50-100 times daily.',
...
export const HITTING_KNOWLEDGE = {
  mechanicsDifferences: {
    fastpitchVsBaseball: {
      ...
      importance:
        'Softball-specific coaching is ESSENTIAL. Applying baseball-only principles damages swing development.',
  ...
  keyMetrics: {
    exitVelocity: {
      description:
        'Primary metric for measuring hitting quality - reflects timing, mechanics, and contact quality',
      benchmarks: {
      ...
      impact:
        '5 mph exit velocity increase correlates to 18-62% higher batting average at MLB level',
      realResults:
        'Rachel Garcia added 11 mph in single 30-minute session. Teams average 7 mph increases with proper training.',
    },
  ...
  essentialMechanics: {
    ...
    commonIssues: {
      casting:
        'Hands drift away from body, creating long swing path. Fix: Keep hands inside, lead with elbow',
  ...
  trainingApproach: {
    drillStructure:
      'Short 3-5 swing rounds with minimal feedback between swings to create game-ready conditions',
    focusAreas:
      'Develop efficient, connected swings that maximize exit velocity through proper sequencing',
...
export const CATCHING_KNOWLEDGE = {
  ...
  blockingProgression: {
    ...
    keyDrills: {
      catchBlockFake:
        'Coach throws from 30 feet, mixing strikes with dirt balls at random to develop reactive blocking',
      shadowBlocking:
        'No ball - coach signals location, catcher assumes blocking position for form check',
      dropBlockRise:
        'Move along line of 6-10 balls, dropping into blocking position at each while keeping glove low',
    },
  },
  ...
  throwingAndPopTime: {
    ...
    training:
      'Master technique before increasing speed. Proper form prevents injury and improves results.',
  },
};
...
export const INFIELD_KNOWLEDGE = {
  ...
  essentialDrills: {
    sideToSide:
      'Shuffle laterally without crossing feet, field grounders between feet while staying low',
    infieldFootwork:
      'Sprint to ball, fire to second, sprint to first, receive and throw home - develops double play footwork and conditioning',
    rapidFireFootwork:
      'Focus on body control and quick transition to throwing position using short steps',
    throwingOnRun: 'Catch-to-throw transitions, coordination, and ball handling',
    handsRoutine:
      'Progress from knees (glove angle focus) to wide base (add footwork) to full movement',
  },
...
export const OUTFIELD_KNOWLEDGE = {
  ...
  throwingMechanics: {
    crowHop: {
      ...
      technique:
        'Leap forward, raise throwing arm knee first then glove knee, throw when back foot lands',
...
export const BIOMECHANICS_FRAMEWORK = {
  kineticChain: {
    ...
    efficiency:
      'Each segment reaches peak velocity as previous segment decelerates, transferring energy',
    leaks:
      'Any break in chain reduces power transfer - most common: hips and shoulders rotating together',
  },

  forceGeneration: {
    groundReactionForce:
      'Push into ground creates equal and opposite force - foundation of all power',
