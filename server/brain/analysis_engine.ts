...
async function analyzeWindmillMechanics(
  ...
  const pitchingDrills = await db.select().from(drills).where(eq(drills.category, 'PITCHING'));
...
async function analyzeSwingMechanics(
  ...
  const hittingDrills = await db.select().from(drills).where(eq(drills.category, 'HITTING'));
...
async function analyzePopTime(videoUrl: string, athleteLevel: string): Promise<AnalysisResult> {
  // Get catching drills for recommendations
  const catchingDrills = await db.select().from(drills).where(eq(drills.category, 'CATCHING'));
...
async function analyzeFieldingMechanics(
  ...
  const fieldingDrills = await db.select().from(drills).where(eq(drills.category, 'FIELDING'));

  // Also get infield/outfield drills
  const infieldDrills = await db.select().from(drills).where(eq(drills.category, 'INFIELD'));
