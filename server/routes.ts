...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
    ...
    try {
      ...
      if (athlete) {
        ...
        await storage.updateAthlete(athlete.id, {
          ...
          graduationYear:
            profileData.athleteGrade === 'college'
              ? new Date().getFullYear() + 4
              : profileData.athleteGrade === '12'
                ? new Date().getFullYear()
                : profileData.athleteGrade === '11'
                  ? new Date().getFullYear() + 1
                  : profileData.athleteGrade === '10'
                    ? new Date().getFullYear() + 2
                    : profileData.athleteGrade === '9'
                      ? new Date().getFullYear() + 3
                      : profileData.athleteGrade === '8'
                        ? new Date().getFullYear() + 4
                        : profileData.athleteGrade === '7'
                          ? new Date().getFullYear() + 5
                          : profileData.athleteGrade === '6'
                            ? new Date().getFullYear() + 6
                            : profileData.athleteGrade === '5'
                              ? new Date().getFullYear() + 7
                              : profileData.athleteGrade === '4'
                                ? new Date().getFullYear() + 8
                                : profileData.athleteGrade === '3'
      ...
      res.json({
        success: true,
        message: 'Parent account and athlete profile created successfully',
      });
    } catch (err) {
      if (err instanceof z.ZodError) {
        return res.status(400).json({ message: err.errors[0].message });
  ...
  app.post('/api/player/checkin', async (req, res) => {
    try {
      ...
      const isArmSore =
        data.sorenessAreas.includes('arm') || data.sorenessAreas.includes('shoulder');
  ...
  app.get(api.athletes.get.path, async (req, res) => {
    const athlete = await storage.getAthlete(
    ...
    try {
      ...
      if (!isAuthorized) {
        ...
        if (coach) {
          ...
          const isMyStudent = relationships.some(
            (rel) => rel.playerId === athlete.userId && rel.status === 'active',
          );
          if (isMyStudent) {
            isAuthorized = true;
          }
        ...
        athletes.map(async (athlete) => {
          ...
          if (athlete.userId) {
            ...
            if (checkin) {
              ...
              const isArmSore =
                lastCheckIn.sorenessAreas.includes('arm') ||
  ...
  app.post('/api/practice-plans', async (req, res) => {
    try {
      ...
      if (!coach)
        return res
          .status(404)
          .json({ message: 'Coach profile not found. Please complete your profile first.' });
  ...
  app.post('/api/analysis/process', async (req, res) => {
    try {
      ...
      const athleteLevel =
        (data.athleteLevel &&
          (validLevels.includes(data.athleteLevel as any)
            ? (data.athleteLevel as any)
            : undefined)) ||
        undefined;
    ...
    try {
      ...
      const schema = z.object({
        ...
        kneeFlexion: z
          ...
          .max(180)
          .nullable()
          .refine((val) => val === null || (val >= 0 && val <= 180), {
    ...
    try {
      ...
      for (const drill of drills) {
        try {
          ...
          errors.push(
            `Failed to import "${drill.name}": ${err instanceof Error ? err.message : 'Unknown error'}`,
          );
        }
      }
  ...
  app.delete('/api/mental-edge/:id', async (req, res) => {
    try {
      await storage.deleteMentalEdge(
        Number(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id),
      );
  ...
  app.delete('/api/player/coaches/:id', async (req, res) => {
    try {
      if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
      await storage.deletePlayerCoachRelationship(
        Number(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id),
      );
  ...
  app.get(
    '/api/coach/review-queue',
    requireRole('team_coach', 'pitching_coach'),
    async (req, res) => {
      try {
        if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
        const userId = (req.user as any).claims.sub;
        ...
        const pendingAssessments = await storage.getAssessmentsByStatus(
  ...
  app.post(
    '/api/coach/assessments/:id/approve',
    requireRole('team_coach', 'pitching_coach'),
    async (req, res) => {
      try {
        if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
  ...
  app.post(
    '/api/coach/assessments/:id/edit-approve',
    requireRole('team_coach', 'pitching_coach'),
    async (req, res) => {
      try {
        if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
  ...
  app.get('/api/team/:teamId/referral-code', async (req, res) => {
    try {
      ...
      const userId = (req.user as any).claims.sub;
      const teamId = Number(
        Array.isArray(req.params.teamId) ? req.params.teamId[0] : req.params.teamId,
      );
      ...
      const students = await Promise.all(
        relationships.map(async (rel) => {
          ...
          return {
            ...
            baselineVideos: needsReview
              ? baselineVideos.map((v) => ({
                  id: v.id,
  ...
  app.post(
    '/api/specialist/roster/:id/archive',
    requireRole('pitching_coach'),
    async (req, res) => {
      try {
        if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
  ...
  app.post('/api/register/complete', async (req, res) => {
    try {
      if (!req.user)
        return res.status(401).json({ message: 'Must be logged in to complete registration' });
    ...
    team_coach: [
      {
        ...
        category: 'hitting',
        title: 'Hitting',
        description: 'Record your swing from the side - show full stance through follow-through',
        focusAreas: ['bat path', 'hip rotation', 'contact point'],
      },
      {
        number: 2,
        category: 'throwing',
        title: 'Throwing',
        description: 'Record your throwing motion from the side - show arm action and release',
        focusAreas: ['arm slot', 'stride length', 'follow-through'],
      },
      {
        number: 3,
        category: 'fielding',
        title: 'Fielding',
        description: 'Record yourself fielding ground balls - show ready position and transfers',
        focusAreas: ['glove position', 'footwork', 'transfer speed'],
      },
      {
        number: 4,
        category: 'pitching_or_catching',
        title: 'Pitching or Catching',
        description: 'Record your pitch (if pitcher) OR a blocking/framing drill (if catcher)',
        focusAreas: ['arm circle', 'leg drive', 'release point'],
      },
    ],
    pitching_instructor: [
      {
        ...
        category: 'fastball',
        title: 'Fastball',
        description: 'Record 3-5 fastballs from the side view - full windmill motion',
        focusAreas: ['arm circle', 'knee drive', 'hip-shoulder separation', 'release point'],
      },
      {
        number: 2,
        category: 'drop_ball',
        title: 'Drop Ball',
        description: 'Record 3-5 drop balls from the side view - focus on spin and release',
        focusAreas: ['wrist snap', 'release angle', 'leg drive'],
      },
      {
        number: 3,
        category: 'change_up',
        title: 'Change-up',
        description: 'Record 3-5 change-ups - show deceptive motion and grip release',
        focusAreas: ['arm speed', 'deception', 'grip release'],
      },
      {
        number: 4,
        category: 'pitchers_choice',
        title: "Pitcher's Choice",
        description: 'Record your specialty pitch (rise ball, curve, screw, etc.)',
        focusAreas: ['spin direction', 'movement', 'command'],
      },
    ],
    catching_instructor: [
      {
        ...
        category: 'framing',
        title: 'Framing',
        description: 'Record yourself receiving 5+ pitches - show glove presentation and stick',
        focusAreas: ['glove angle', 'subtle movement', 'stick presentation'],
      },
      {
        number: 2,
        category: 'blocking',
        title: 'Blocking',
        description: 'Record yourself blocking balls in the dirt - show stance and body position',
        focusAreas: ['drop mechanics', 'glove position', 'body angle'],
      },
      {
        number: 3,
        category: 'transfer',
        title: 'Transfer (Pop-time)',
        description: 'Record catch-to-throw transfer - show footwork and arm action',
        focusAreas: ['transfer speed', 'footwork', 'arm slot', 'pop-time'],
      },
      {
        number: 4,
        category: 'bunt_coverage',
        title: 'Bunt Coverage',
        description: 'Record fielding bunts - show explosiveness and throw accuracy',
        focusAreas: ['first step', 'barehand technique', 'throw accuracy'],
      },
    ],
  };
  ...
  app.get('/api/player/onboarding', async (req, res) => {
    try {
      ...
      const onboardingType =
        (onboarding.onboardingType as keyof typeof VIDEO_PROMPTS) || 'team_coach';
  ...
  app.post('/api/player/baseline-video', async (req, res) => {
    try {
      ...
      if (['fastball', 'drop_ball', 'change_up', 'pitchers_choice'].includes(data.videoCategory)) {
        ...
        ['framing', 'blocking', 'transfer', 'bunt_coverage'].includes(data.videoCategory)
      ) {
        skillType = 'CATCHING';
  ...
  app.post(
    '/api/specialist/baseline/:playerId/approve',
    requireRole('pitching_coach'),
    async (req, res) => {
      try {
        if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
        ...
        const relationships = await storage.getCoachPlayers(coach.id);
        const isMyStudent = relationships.some(
          (rel) => rel.playerId === playerId && rel.status === 'active',
        );
        if (!isMyStudent) {
          return res
            .status(403)
            .json({ message: 'You can only approve baselines for your own students' });
        }
        ...
        await storage.updatePlayerOnboarding(playerId, {
          ...
          nextVideoPromptDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split('T')[0],
        });
  ...
  app.post('/api/stats/import', isAuthenticated, async (req, res) => {
    try {
      ...
      const savedStats = await storage.createGameChangerStats({
        ...
        firstPitchStrikePercent: stats.firstPitchStrikePercent
          ? stats.firstPitchStrikePercent
          : null,
  ...
  app.post('/api/stripe/checkout', isAuthenticated, async (req, res) => {
    try {
      ...
      if (!customerId) {
        ...
        const name =
          `${userClaims.first_name || ''} ${userClaims.last_name || ''}`.trim() || undefined;
  ...
  app.post('/api/team-stats/import', isAuthenticated, async (req, res) => {
    try {
      ...
      for (const row of dataRows) {
        ...
        if (playerOps > 0.7 || playerAvg > 0.3) {
