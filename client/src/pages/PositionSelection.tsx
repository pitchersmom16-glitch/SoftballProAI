...
const POSITIONS = [
  ...
  { id: '1B', name: 'First Base', category: 'infield', description: 'Corner Infielder' },
  {
    ...
    name: 'Second Base',
    category: 'infield',
    description: 'Middle Infielder, Double Play',
  },
  ...
  { id: 'LF', name: 'Left Field', category: 'outfield', description: 'Pull Power, Left Side' },
  {
    ...
    name: 'Center Field',
    category: 'outfield',
    description: 'Center of Attention, Speed',
  },
  { id: 'RF', name: 'Right Field', category: 'outfield', description: 'Right Side, Arm Strength' },
  {
    ...
    category: 'hitting',
    description: 'Pure Hitter, No Fielding',
  },
];
...
export default function PositionSelection() {
  ...
  const getCategoryInfo = (category: string) => {
    return (
      POSITION_CATEGORIES[category as keyof typeof POSITION_CATEGORIES] ||
      POSITION_CATEGORIES.infield
    );
  };
      ...
      <Card className="w-full max-w-4xl bg-blacklightBase border-electricPink/30">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-cyberBlue">
            Select your primary position and optionally a secondary position for comprehensive
            training
          </CardDescription>
              ...
              {POSITIONS.filter((pos) => pos.id !== selectedPrimary).map((position) => {
                ...
                return (
                  <button
                    key={position.id}
                    onClick={() =>
                      setSelectedSecondary(position.id === selectedSecondary ? null : position.id)
                    }
                    className={`p-4 rounded-lg border-2 transition-all duration-200 text-left ${
                      isSelected
            ...
            <div className="bg-charcoal2/50 rounded-lg p-4 border border-electricPink/20">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedPrimary && (
                  <Badge
                    variant="secondary"
                    className="bg-electricPink/20 text-electricPink border-electricPink/30"
                  >
                ...
                {selectedSecondary && (
                  <Badge
                    variant="secondary"
                    className="bg-ultraviolet/20 text-ultraviolet border-ultraviolet/30"
                  >
