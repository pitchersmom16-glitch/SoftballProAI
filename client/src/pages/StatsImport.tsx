...
export default function StatsImport() {
  ...
  const statCards = [
    ...
    { label: 'K%', value: parsedStats?.kPercent, icon: Zap, color: 'text-yellow-400' },
    {
      ...
      value: parsedStats?.firstPitchStrikePercent,
      icon: Check,
      color: 'text-cyan-400',
    },
    {
      label: 'Exit Velocity',
      value: parsedStats?.exitVelocity,
      icon: TrendingUp,
          ...
          <CardContent className="space-y-6">
              ...
              {file ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Supports: Name, Pos, AVG, OPS, ERA, WHIP, K%, First Pitch Strike %, Exit
                    Velocity
                  </p>
                </div>
              )}
            ...
            {parsedStats && (
              <div className="space-y-4">
                <h3 className="font-semibold text-lg" data-testid="text-preview-title">
                  Preview Parsed Stats
                </h3>

                {parsedStats.name && (
                  <div className="flex items-center gap-2 text-foreground">
                    <span className="text-muted-foreground">Player:</span>
                    <span className="font-medium" data-testid="text-player-name">
                      {parsedStats.name}
                    </span>
                    {parsedStats.pos && (
        ...
        <Card className="border-pink-500/30 bg-card/50 backdrop-blur">
          ...
          <CardContent>
            <ol className="space-y-3 text-muted-foreground">
              ...
              <li className="flex items-start gap-3">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-pink-500/20 text-pink-400 flex items-center justify-center text-sm font-bold">
                  4
                </span>
